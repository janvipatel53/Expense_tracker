...
def index():
    ...
    budget_row = conn.execute(
        "SELECT monthly_limit FROM budget WHERE user_id = ?",
        (user_id,)
    ).fetchone()
    ...
    if monthly_budget > 0:
        usage = total_spent / monthly_budget

        if usage >= 1:
            budget_warning = "Budget exceeded!"
        elif usage >= 0.9:
            budget_warning = "Warning: 90% budget reached!"
...
def set_budget():
    user_id = session["user_id"]
    monthly_limit = request.form["monthly_limit"].strip()
    ...
    try:
        ...
        existing = cursor.execute(
            "SELECT user_id FROM budget WHERE user_id = ?",
            (user_id,)
        ).fetchone()

        if existing:
            cursor.execute(
                "UPDATE budget SET monthly_limit = ? WHERE user_id = ?",
                (float(monthly_limit), user_id)
            )
        else:
            cursor.execute(
                "INSERT INTO budget (user_id, monthly_limit) VALUES (?, ?)",
                (user_id, float(monthly_limit))
            )
...
def api_expenses():
    ...
    conn = get_db_connection()
    rows = conn.execute(
        "SELECT id, date, category, amount, description FROM expenses WHERE user_id = ? ORDER BY id DESC",
        (user_id,)
    ).fetchall()
    conn.close()
